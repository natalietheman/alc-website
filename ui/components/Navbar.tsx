  ...
  return (
    <div className="fixed z-40 top-0 left-0 right-0 px-4 lg:pl-[104px] lg:pr-6 lg:px-8 flex flex-row items-center justify-between w-full py-4 text-sm text-black dark:text-white/70 border-b bg-light-primary dark:bg-dark-primary border-light-100 dark:border-dark-200">
      ...
      <div className="hidden lg:flex flex-row items-center space-x-4">
        <ALCLogo className="h-8 w-24" />
        <div className="flex flex-row items-center space-x-2">
          <ChatHepLogo className="h-6 w-6" />
          <span className="font-semibold">ChatHEP</span>
        </div>
      </div>
      <p className="hidden lg:flex absolute left-1/2 transform -translate-x-1/2">{title}</p>
